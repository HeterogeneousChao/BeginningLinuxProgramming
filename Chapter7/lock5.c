...
int main()
{
	...
	printf("Process %d, trying F_RDLCKM region %d to %d\n", getpid(), 
	...
	if (res == -1) {
		...
		printf("Process %d - obtained lock regionn\n", getpid());
	}
	...
	printf("Process %d, trying F_WRLCK, region %d to %d", getpid(),
		(int)region_to_lock.l_start, (int)(region_to_lock.l_start + region_to_lock.l_len));
	res = fcntl(file_desc, F_SETLK, &region_to_lock);
	...
	printf("Process %d, tyring F_WRLCK with wait, region %d to %d\n", getpid(),
		(int)region_to_lock.l_start, (int)(region_to_lock.l_start + region_to_lock.l_len));

	res = fcntl(file_desc, F_WRLCK, &region_to_lock);
