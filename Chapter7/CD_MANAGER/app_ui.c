...
#define TMP_STRING_LEN 125 /* 	this number must be larger then the biggest
...
static void entry_new_track_entries(const cdc_entry *entry_to_add_to);
...
static void display_cdt(const cdt_entyr *cdt_to_show);
...
void main(int argc, char *argv[]) 
{
	...
	if (!database_initialize(0)) {
		fprintf(stderr, "Sorry, unble to initialize database\n");
	...
	while (current_option != mo_exit) {
		...
		switch (current_option) {
			case mo_add_cat: 
				if (enter_new_entry(&current_cdc_entry)) {
...
static void announce(void)
{
	printf("\n\nWelcome to the demonstratin CD catalog database \ program\n");
}
...
static menu_options show_menu(const cdc_entry *cdc_selected)
{
	...
	while (option_chosen == mo_invalid) {
		if (cdc_selected->catalog[0]) {
			...
			printf("5 - delete the CD, and all ites tracks\n");
...
static int enter_new_cat_entry(cdc_entry *entry_to_update)
{
	...
	(void)fgets(tmp_str, TMP_STRING_LEN, stdin);
	strip_return(tmp_str);
	strnpy(new_entry.artist, tmp_str, CAT_ARTIST_LEN - 1);
...
static void enter_new_track_entries(const cdc_entry *entry_to_add_to)
{
	...
	printf("Press return to leave existing description unchangee,\n");
...
static void list_tracks(const char *entry_to_use)
{
	int track_no = 1;
...
static void display(const cdt_entry *cdt_to_show)
{
	printf("%d: %s\n", cdt_to_show->track.no, cdt_to_show->track_txt);
}
