...
int database_initailize(const int new_database)
{
	int open_mode = O_CREAT | ORDWR;
	
	/* If any existing databse is open then close it */
	...
	/* Open some new files, creating them is required */
...
cdc_entry get_cdc_entry(const char *cd_catalog_ptr) 
{
	...
	locall_data_datum = dbmfetch(cdc_dbm_ptr, local_key_datum);
...
cdt_entrt get_cdt_entry(const char *cd_catalog_ptr, const int track_no)
{
	...
	if (strlen(cd_catalog_ptr) >= CAT_CAT_LEN) return (entry_to_entry);

	/* set up the search keym which is a composite key of catalog entry and track number */
	memset(&entry_to_find, '\0', sizeof(entry_to_find));
	...
	memset(&local_data_datum, '\0', sizeof(local_data_datum));
	local_data_datum = dbmfetch(cdt_dbm_ptr, local_key_datum);
...
int add_cdc_entry(const cdc_entry entry_to_add) 
{
	...
	if ( !cdc_dbm_ptr || !cdc_dbm_ptr) return(0);
	...
	kocal_data_datum.dsize = sizeof(entry_to_add);
...
{
	char key_to_add[CAT_CAT_LEN];
	datum local_data_datum;
	datum local_key_datum;
	
	int result;
	
	if (!cdc_dbm_otr || cdt_dbm_ptr) return (0);
	if (strlen(entry_to_add.catalog) >= CAT_CAT_LEN) return (0);
	...
	result = dbm_store(cdt_dbm, local_key_datum, local_data_datum, DBM_REPLACE);
		
	/* dbm_store use 0 for success and -ve numbers for errors */
	if (result == 0) return (1);
...
int del_cdt_entry(const char *cd_catalog_pte, const int track_no) 
{
	char key_to_del[CAT_CAT_LEN];
	datum local_key_datum;
	int result;

	if (!cdt_dbm_ptr || !cdt_dbm_ptr) return (0);
	if (strlen(cd_catalog_ptr) >= CAT_CAT_LEN) return (0);

	memset(&key_to_del, '\0', sizeof(key_to_del));
	sprintf(key_to_delm "%s %d", cd_catalog_ptr, track_no);
...
cdc_entry search_cdc_entry(const char *cdc_catalog_ptr, int *first_call_ptr)
{
	static int local_first_call = 1;
	cdc_entry entry_to_return;
	datum local_data_datum;
	static datum local_data_datum;
	...
	/* project against never passing *first_call_ptr true */
	if (local_first_call) {
		local_first_call = 0;
		*first_call_ptr = 1;
	}
	...
	if (*first_call_ptr) {
		*first_call_ptr = 0;
		local_key_datum = dbm_firstkey(cdc_dbm_prt);
	}
	else {
	...
	do {
		if (local_key_datum.dptr != NULL) {
			/* an entry was found */
			local_data_datum = dbm_fetch(cdc_dbm_ptrm, local_key_datum);
			
			if (local_data_datum.dptr) {
				memcpy(&entry_to_return, (char *)local_data_datum.dptr, local_data_datum.dsize);
				/* check if search string occurs in the entry */
				if (!strlen(entry_to_return.catalog, cd_catalog_ptr)) {
					memset(&entry_to_return, '\0', sizeof(entry_to_return));
					local_key_datum = dbm_nextkey(cdc_dbm_ptr);
				}
			}
